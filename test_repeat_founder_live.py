"""One-off diagnostic: verify repeat-founder enrichment against REAL
director appointment records from companies already processed today,
without re-running the whole pipeline or touching data/seen.json. Costs
nothing extra -- Companies House is free, no separate auth needed beyond
the existing CH_API_KEY -- and makes zero Anthropic API calls.

Usage:
    python test_repeat_founder_live.py                    # uses today's data/briefs/*.json
    python test_repeat_founder_live.py --date 2026-07-28
"""
...
from scanner import config, fetch, generate, repeat_founder
...
def main(target_date: str) -> None:
    ...
    for number in numbers:
        ...
        if not officers:
            ...
            continue
        any_officers = True
        enriched = repeat_founder.enrich_officers(officers, exclude_company_number=number)

        # Collapse the same person appearing twice (e.g. director + company
        # secretary) into one line, same as test_orcid_live.py/test_gtr_live.py.
        by_name: dict[str, dict] = {}
        for o in enriched:
            name = o.get("name", "")
            entry = by_name.setdefault(name, {"roles": [], "repeat_founder": o.get("repeat_founder")})
        ...
        for name, info in by_name.items():
            ...
            note = generate._format_repeat_founder(info["repeat_founder"])
            if note is None:
                status = (info["repeat_founder"] or {}).get("status", "n/a")
