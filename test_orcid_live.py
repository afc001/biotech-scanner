"""One-off diagnostic: verify ORCID enrichment against REAL director names
...
ORCID are both free -- and makes zero Anthropic API calls.

Usage:
    python test_orcid_live.py                    # uses today's data/briefs/*.json
    python test_orcid_live.py --date 2026-07-28
"""
...
from scanner import config, fetch, generate, orcid
...
def main(target_date: str) -> None:
    ...
    if not orcid.enabled():
        print("ORCID_CLIENT_ID / ORCID_CLIENT_SECRET not set -- nothing to test.")
        return
    ...
    for number in numbers:
        ...
        if not officers:
            ...
            continue
        any_officers = True
        enriched = orcid.enrich_officers(officers)

        # Collapse the same person appearing twice (e.g. director + company
        # secretary) into one line -- there's only one ORCID result to show,
        # even though Companies House lists them per-role.
        by_name: dict[str, dict] = {}
        for o in enriched:
            name = o.get("name", "")
            entry = by_name.setdefault(name, {"roles": [], "orcid": o.get("orcid")})
        ...
        for name, info in by_name.items():
            ...
            note = generate._format_orcid(info["orcid"])
            if note is None:
                status = (info["orcid"] or {}).get("status", "n/a")
